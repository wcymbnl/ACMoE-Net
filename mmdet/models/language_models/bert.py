...
class BertModel(BaseModel):
    ...
    def __init__(self,
                 ...
                 use_mamba: bool = False,   #改
                 mamba_cfg: dict = None,    #改
                 **kwargs) -> None:
        ...
        self.language_backbone = nn.Sequential(
            OrderedDict([('body',
                          BertEncoder(
                              ...
                              use_mamba=use_mamba,  #改
                              mamba_cfg=mamba_cfg   #改
                              ))]))
...
class BertEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.mamba_cfg = {} if mamba_cfg is None else mamba_cfg

        if self.use_mamba:
            from .mamba2_simple import Mamba2Simple
            self.mamba = Mamba2Simple(
                d_model=self.language_dim,
                **self.mamba_cfg,
            )
            self.mamba_norm = nn.LayerNorm(self.language_dim)
        #####
    ...
    def forward(self, x) -> dict:
        ...
        valid_token_mask = x.get('text_token_mask', None)
        ...
        if valid_token_mask is not None:
            embedded = features * valid_token_mask.unsqueeze(-1).float().to(
                features.dtype)
        elif mask.dim() == 2:
        ...
        if self.use_mamba:
            ...
            if position_ids is not None and valid_token_mask is not None:
                # Reset the scan when a new valid sub-sentence starts.
                reset_points = (position_ids == 0) & valid_token_mask
                seq_idx = torch.cumsum(reset_points.to(torch.int32), dim=-1) - 1
                seq_idx = seq_idx.masked_fill(~valid_token_mask, -1)

            if seq_idx is not None:
                seq_idx = seq_idx.to(torch.int32)

            mamba_out, _ = self.mamba(embedded, seq_idx=seq_idx)

            if valid_token_mask is not None:
                mamba_out = mamba_out * valid_token_mask.unsqueeze(-1).to(
                    mamba_out.dtype)
            elif mask.dim() == 2:
                mamba_out = mamba_out * mask.unsqueeze(-1).to(mamba_out.dtype)

            embedded = self.mamba_norm(embedded + mamba_out)
