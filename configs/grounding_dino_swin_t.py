...
clean_caption=True
use_prompt_adapter=True    #moae的总开关，为False为原来的模式
prompt_adapter_cfg=dict(
    use_dynamic_prompt=True,  #动态prompt开关
    use_instruct_prompt=True,  #pm开关
    moe_n_experts=4,           #4个专家
...
model = dict(
    ...
    use_p5_input=True,
    use_p4_input=True,
    use_vision_mask=True,    #改   用来生成mask
    vision_mask_level=-2,    #改
    use_text_guided_refine=True,  #改
    text_guided_level=-2,   #改
    text_guided_cfg=dict(   #改
        d_model=256,       #改
        forward_type='m0',  #改
        with_initial_state=False,  #改
        channel_first=False,   #改
    ),   #改
    ...
    prompt_adapter_importance_loss_weight=0,    #如果开了pd，就别设置0  开了以后有一个loss会变成nan
    ...
    language_model=dict(
        ...
        use_mamba=True,   #添加mamba  改
        mamba_cfg=dict(   #添加mamba  改
            d_state=64,
            d_conv=4,
            expand=2,
            headdim=128,
...
default_hooks = dict(
    checkpoint=dict(by_epoch=False, interval=10000, max_keep_ckpts=30),  #改30000
...
load_from = '/media/ciisr/Data2/ldl/llmdet/LLMDet/tiny.pth'